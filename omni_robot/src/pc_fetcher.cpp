...
pc_fetcher::pc_fetcher(){
   ...
   ros::Subscriber _imSub1 = _slaveRobot.subscribe("/lsd_slam_r1/keyframes", 30, &pc_fetcher::im1Callback, this);
   ros::Subscriber _imSub2 = _slaveRobot.subscribe("/lsd_slam_r2/keyframes", 30, &pc_fetcher::im2Callback, this);
    
    omniPub = _slaveRobot.advertise<sensor_msgs::PointCloud2>("/omnicloud", 5);
...
void pc_fetcher::im1Callback(slave_robot::keyframeMsgConstPtr msg){
    ROS_INFO("received callback from Robot1;");
    omnicloud2.data.clear();
    omnicloud.points.clear();
	...
	for(int x=1;x<_width-1;x++){
	    ...
	    _tempbuffer.y=pt[1];
            _tempbuffer.z=pt[2];
	    omnicloud.points.push_back(_tempbuffer);
	}

    sensor_msgs::convertPointCloudToPointCloud2(omnicloud,omnicloud2);	
    ROS_INFO("pc calling back...");
    omnicloud2.header.seq=kfIndex;
    omnicloud2.header.stamp= timeStamp;
    omnicloud2.header.frame_id="world";
...
// Filling in Similarity Trafo Info
    ...
    omniPub.publish(omnicloud2);
}
void pc_fetcher::im2Callback(slave_robot::keyframeMsgConstPtr msg){
    
    ROS_INFO("received callback from Robot2;");
    geometry_msgs::Point32 _tempbuffer;
    ros::Time timeStamp(msg->time);
    _fx = msg->fx;
    _fy = msg->fy; 
